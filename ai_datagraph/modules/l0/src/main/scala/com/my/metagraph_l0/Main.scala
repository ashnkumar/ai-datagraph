...
import com.my.shared_data.lib.{JsonBinaryCodec, PostgresService}
...
object Main
  ...
  override def dataApplication: Option[Resource[IO, BaseDataApplicationL0Service[IO]]] = (for {
    ...
    implicit0(jsonSerializer: JsonSerializer[IO])    <- JsonBinaryCodec.forSync[IO].asResource
    ...
    postgresService                                  <- PostgresService.make[IO](
      config.postgresDatabase.url,
      config.postgresDatabase.user,
      config.postgresDatabase.password
    )
    l0Service <- ML0Service.make[IO](postgresService).asResource
  } yield l0Service).some

  override def rewards(implicit
    sp: SecurityProvider[IO]
  ): Option[
    Rewards[IO, CurrencySnapshotStateProof, CurrencyIncrementalSnapshot, CurrencySnapshotEvent]
  ] =
